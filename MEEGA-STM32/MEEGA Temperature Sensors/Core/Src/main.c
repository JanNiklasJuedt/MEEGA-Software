...
#define CMD_SYNC 0x42

//Transfer length
#define TxPACKET_LENGTH 22 //3 bytes from 24bit ---- CHANGE if needed
/* USER CODE END PV */
...
void createtxPacket(void) {
	...
	/*ADCConfig(CS_T1_PIN,2,2,1,0); //Change Channel and gain for Thermoresistor
	//CS3 24bit - TC1046
	if(ADCRegister(CS_T1_PIN)==1) {
		HAL_GPIO_WritePin(CS_SENSORS_PORT, CS_T1_PIN, GPIO_PIN_RESET);
		HAL_SPI_Transmit(&hspi2, dataREG, 1, HAL_MAX_DELAY);
		...
		//HAL_SPI_TransmitReceive(&hspi2, dataREG, rxBuf, 4, HAL_MAX_DELAY);
		HAL_GPIO_WritePin(CS_SENSORS_PORT, CS_T1_PIN, GPIO_PIN_SET);
		COLD_JUNCTION = Sensors24bits(rxBuf[1], rxBuf[2], rxBuf[3]); //RAW Output
	}
	else SensorError = 1;
