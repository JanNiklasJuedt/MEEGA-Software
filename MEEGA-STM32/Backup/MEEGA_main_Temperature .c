...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body for Temperature Sensors
  ******************************************************************************
  * @attention
  *
...
#define CS_TAcc_PIN GPIO_PIN_3
#define CS_T0_PIN GPIO_PIN_4
#define CS_T1_PIN GPIO_PIN_5
#define CS_T2_PIN GPIO_PIN_6
#define CS_T3_PIN GPIO_PIN_7
#define CS_PT100_PIN GPIO_PIN_8

//CMD from RPi to read sensors data
#define CMD_READ 0xA1

//Transfer length
#define TxPACKET_LENGTH 19 //3 bytes from 24bit ---- CHANGE if needed
/* USER CODE END PV */
...
static uint8_t rxBuf24[3];

static uint8_t txPacket[TxPACKET_LENGTH];
static uint8_t txLatest[TxPACKET_LENGTH];
static uint8_t rxCMD;

//Communication Register
//static uint8_t comCMD = 0x40;
//static uint8_t dataREG = 0x58;
...
void ADCReset(uint16_t CS_Pin) {
	uint8_t resetCMD[4] = {0xFF, 0xFF, 0xFF, 0xFF};
...
void ADCConfig(uint16_t CS_Pin, uint8_t AIN_CH) {
	uint8_t modeCMD = 0x08;
	uint8_t modeData[2] = {0x00, 0x0A};
...
uint8_t ADCRegister(uint16_t CS_Pin) {
	uint8_t comCMD = 0x40; //0b0100_0000 8bit Status Register During a Read Operation Tab.12
	uint8_t statREG = 0xFF; //Status Register
	uint32_t timeout = HAL_GetTick() + 5; //7ms Timeout CHANGEABLE
	while ((statREG & 0x80) && HAL_GetTick() < timeout) {	//0x80 mask out for checking SR7 RDY
		HAL_GPIO_WritePin(CS_SENSORS_PORT, CS_Pin, GPIO_PIN_RESET);
		HAL_SPI_Transmit(&hspi2, &comCMD, 1, HAL_MAX_DELAY);
		HAL_SPI_Receive(&hspi2, &statREG, 1, HAL_MAX_DELAY);
		//HAL_SPI_TransmitReceive(&hspi2, comCMD, statREG, 2, HAL_MAX_DELAY);
		HAL_GPIO_WritePin(CS_SENSORS_PORT, CS_Pin, GPIO_PIN_SET);
	}
	if ((statREG & 0x80) == 0) { // RDY = 0 ? conversion ready
		if ((statREG & 0x40) == 0) { // ERR = 0 ? no error
...
void createtxPacket(void) {
	uint32_t T_Accumulator_ADC = 0, T0_ADC = 0, T1_ADC = 0, T2_ADC = 0, T3_ADC = 0, PT100_ADC = 0;
	uint8_t SensorError = 0; //Error Flag
	uint8_t dataREG = 0x58; //0b0101_1000 Data Register 24bit Tab.12
	...
	if(ADCRegister(CS_TAcc_PIN)==1) {
		...
		HAL_SPI_Receive(&hspi2, rxBuf24, 3, HAL_MAX_DELAY);	//read data
		HAL_GPIO_WritePin(CS_SENSORS_PORT, CS_TAcc_PIN, GPIO_PIN_SET);		//CS HIGH to end communication
		T_Accumulator_ADC = Sensors24bits(rxBuf24[0], rxBuf24[1], rxBuf24[2]); //RAW Output
	}
	else SensorError = 1;
	...
	if(ADCRegister(CS_T0_PIN)==1) {
		...
		HAL_SPI_Transmit(&hspi2, &dataREG, 1, HAL_MAX_DELAY);
		HAL_SPI_Receive(&hspi2, rxBuf24, 3, HAL_MAX_DELAY);
		HAL_GPIO_WritePin(CS_SENSORS_PORT, CS_T0_PIN, GPIO_PIN_SET);
		T0_ADC = Sensors24bits(rxBuf24[0], rxBuf24[1], rxBuf24[2]); //RAW Output
	}
	else SensorError = 1;
	...
	if(ADCRegister(CS_T1_PIN)==1) {
		...
		T1_ADC = Sensors24bits(rxBuf24[0], rxBuf24[1], rxBuf24[2]); //RAW Output
	}
	else SensorError = 1;
	...
	if(ADCRegister(CS_T2_PIN)==1) {
		...
		T2_ADC = Sensors24bits(rxBuf24[0], rxBuf24[1], rxBuf24[2]); //RAW Output
	}
	else SensorError = 1;
	...
	if(ADCRegister(CS_T3_PIN)==1) {
		...
		T3_ADC = Sensors24bits(rxBuf24[0], rxBuf24[1], rxBuf24[2]); //RAW Output
	}
	else SensorError = 1;
	...
	if(ADCRegister(CS_PT100_PIN)==1) {
		...
		PT100_ADC = Sensors24bits(rxBuf24[0], rxBuf24[1], rxBuf24[2]); //RAW Output
	}
	else SensorError = 1;

	/*
	if ((T_Accumulator_ADC == 0 || T_Accumulator_ADC == 0xFFFFFF) ||
		(T0_ADC == 0 || T0_ADC == 0xFFFFFF) ||
		(T1_ADC == 0 || T1_ADC == 0xFFFFFF) ||
		(T2_ADC == 0 || T2_ADC == 0xFFFFFF) ||
		(T3_ADC == 0 || T3_ADC == 0xFFFFFF) ||
		(PT100_ADC == 0 || PT100_ADC == 0xFFFFFF)) {
		SensorError = 1;
	}
 	 */
...
int main(void)
{
  ...
  ADCReset(CS_TAcc_PIN);  ADCConfig(CS_TAcc_PIN,1);
  ADCReset(CS_T0_PIN);    ADCConfig(CS_T0_PIN,1);
  ADCReset(CS_T1_PIN);    ADCConfig(CS_T1_PIN,1);
  ADCReset(CS_T2_PIN);    ADCConfig(CS_T2_PIN,1);
  ADCReset(CS_T3_PIN);    ADCConfig(CS_T3_PIN,1);
  ADCReset(CS_PT100_PIN); ADCConfig(CS_PT100_PIN,1);

  uint32_t last_UpdateTime = 0;
  const uint32_t RefreshData_Timeout = 1; //refresh every 10ms -CHANGEABLE-
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  if(HAL_GetTick() - last_UpdateTime >= RefreshData_Timeout) {
		  HAL_GPIO_TogglePin(LEDs_PORT,LED2);
		  createtxPacket();
		  memcpy(txLatest, txPacket, TxPACKET_LENGTH);
		  last_UpdateTime = HAL_GetTick();
	  }

	  if (HAL_GPIO_ReadPin(CS_RPi_PORT, CS_RPi_PIN) == GPIO_PIN_RESET) {
		  if(HAL_SPI_Receive(&hspi1, &rxCMD, 1, 20) == HAL_OK) { //50ms timeout maximum request period
			  if(rxCMD == CMD_READ) {
				  HAL_SPI_Transmit(&hspi1, txLatest, TxPACKET_LENGTH, HAL_MAX_DELAY);
				  HAL_Delay(1);
			  }
		  }
	  }
	  ...
	  //TEST PACKET
	  /*
	  while(HAL_GPIO_ReadPin(CS_RPi_PORT, CS_RPi_PIN) != GPIO_PIN_RESET) HAL_Delay(1);
	  if(HAL_SPI_Receive(&hspi1, &rxCMD, 1, 20) == HAL_OK) { //50ms timeout maximum request period
		  if(rxCMD == CMD_READ) {
			  HAL_SPI_Transmit(&hspi1, txLatest, TxPACKET_LENGTH, HAL_MAX_DELAY);
			  HAL_Delay(1);
